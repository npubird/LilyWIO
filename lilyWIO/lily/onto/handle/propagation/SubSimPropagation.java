...
//				// 估计
//				if (max>0){
//					cnptSimK1[i][j] = cnptSimK1[i][j]* (tArray[i][j] / max)*(tArray[i][j] / max);
//				}				
//			}
//		}
//		
...
//				// 估计
//				if (max>0){
//					propSimK1[i][j] = propSimK1[i][j]*(pArray[i][j] / max) *(pArray[i][j] / max);
//				}				
//			}
//		}
				...
				for (Iterator ity=t_Stm.iterator();ity.hasNext();){
					...
					//都不是元语的情况
						...
						if (typeA==typeB){
							//相同的类型，需要确定相似度
							/*判断当前tpair是否包含该对*/
							if (rawPairs.contains(subA.toString()+subB.toString())){
								/*已经包含,采用计算的相似度*/
								simS=((Double)rawPairSim.get(subA.toString()+subB.toString())).doubleValue();
							}
							else{
								/*没有包含,采用*/
								simS=getElmSim(subA,subB,typeA);
					...
					//都不是元语的情况
						...
						if (typeA==typeB){
							//相同的类型，需要确定相似度
							/*判断当前tpair是否包含该对*/
							if (rawPairs.contains(propA.toString()+propB.toString())){
								/*已经包含,采用计算的相似度*/
								simP=((Double)rawPairSim.get(propA.toString()+propB.toString())).doubleValue();
							}
							else{
								/*没有包含,采用*/
								simP=getElmSim(propA,propB,typeA);
					...
					//都不是元语的情况
						...
						if (typeA==typeB){
							//相同的类型，需要确定相似度
							/*判断当前tpair是否包含该对*/
							if (rawPairs.contains(objA.toString()+objB.toString())){
								/*已经包含,采用计算的相似度*/
								simO=((Double)rawPairSim.get(objA.toString()+objB.toString())).doubleValue();
							}
							else{
								/*没有包含,采用*/
								simO=getElmSim(objA,objB,typeA);
					...
					if(simElmNum>=2 && !rawTriples.contains(stA.toString()+stB.toString())){
						weight+=5.0;
						TriplePair tpair=new TriplePair();
						tpair.tripleA = stA;
						tpair.tripleB = stB;
	...
	private void cnptSimPropagation(int times) {
		for(int i=0;i<s_cnptNum;i++){
			for (int j=0;j<t_cnptNum;j++){
				...
				if (s_cnptName[i].equals("Academic")&& t_cnptName[j].equals("zdazsx")){
					System.out.println("debug");
				}				
				
				System.out.println(s_cnptName[i]+i+"--"+j+t_cnptName[j]+"相似传播：");
								
				//候选相似三元组对
				Set candiTPSet=new HashSet();
				candiTPSet=getSimTripleCandidate(s_cnptSubG[i].stmList,t_cnptSubG[j].stmList );
...
//				}
				...
				while (sGUpdate && ct<6){
					...
					if (sGUpdate && ct<6){
						//更新子图
						updateTripleCandidate(s_cnptSubG[i].stmList,t_cnptSubG[j].stmList,
								candiTPSet,edgeMap);
						lt=consTriplePairGraph(candiTPSet);
						gNodes=(HashSet)lt.get(0);
						gEdges=(HashSet)lt.get(1);
	...
	private void propSimPropagation(int times) {
		for(int i=0;i<s_propNum;i++){
			for (int j=0;j<t_propNum;j++){
				...
				if (s_propName[i].equals("communications") && t_propName[j].equals("organization")){
					System.out.println("debug");
				}
								
				//候选相似三元组
				Set candiTPSet=new HashSet();
				candiTPSet=getSimTripleCandidate(s_propSubG[i].stmList,t_propSubG[j].stmList );
				
				System.out.println(s_propName[i]+i+"--"+j+t_propName[j]+"相似传播：");
...
//				}
				...
				while (sGUpdate && ct<8){
					...
					if (sGUpdate){
						//更新子图
						updateTripleCandidate(s_propSubG[i].stmList,t_propSubG[j].stmList,
								candiTPSet,edgeMap);
						lt=consTriplePairGraph(candiTPSet);
						gNodes=(HashSet)lt.get(0);
						gEdges=(HashSet)lt.get(1);
